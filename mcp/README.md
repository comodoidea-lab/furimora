...
```
CD・DVD・ブルーレイ > DVD > アニメ > CD・DVD・ブルーレイ > DVD > アニメ
```

メルカリの商品ページは**パンくずを 2 回描画する**（同じ `href` の組が 2 つ）。
旧い抽出は両方を拾っていた。実データのバックアップで**下書き 50 件中 50 件**が該当し、
うち 7 件は先頭に数字だけの断片（`1 > …`）も付いていた。**在庫データは無傷**
（在庫の `category` はパンくずを使っていない）。

- 拡張側は修正済み（`content.js`。`/search?category_id=N` のリンクだけを category_id で
  重複排除して拾う）。**反映には拡張の再読み込みが必要**
- 保存済みの下書きは値が壊れたままなので、読み取り時に `normalizeCategoryPath()` で直す。
  **推測はしない。** 直したのは「経路がそのまま 2 回繰り返されている」「先頭が数字だけ」の
  機械的に確定できる崩れだけで、直した内容は `categoryFixes` と `needsHuman` に必ず出す
